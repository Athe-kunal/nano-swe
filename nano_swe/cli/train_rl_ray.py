...
from nano_swe.trainer.algorithm.experience import get_model_parallel_size
...
def train(args):
    ...
    from nano_swe.trainer.placement import model_placement_strategy
    from nano_swe.trainer.vllm import create_vllm_engines
    from nano_swe.trainer.workers.actor_group import RayActorGroup, ReferenceModelActor
    from nano_swe.trainer.workers.policy_actor import PolicyModelActor
    from nano_swe.utils import get_strategy
    ...
    if vllm_engines:
        from nano_swe.trainer.rollout.router import create_vllm_router
    ...
    from nano_swe.trainer.rl_trainer import RLTrainer
    ...
    if has_critic:
        from nano_swe.trainer.workers.critic_actor import CriticModelActor
...
if __name__ == "__main__":
    ...
    from nano_swe.cli.common_args import (
        add_ckpt_args,
        add_fsdp_args,
        add_logger_args,
        add_optimizer_args,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="YAML recipe (nested keys, e.g. actor: {model_name_or_path: ...}) applied as this "
        "run's defaults; any flag also passed explicitly on the command line still overrides it.",
    )
    ...
    # Two-pass: read --config first and fold its values into this parser's defaults, then do the
    # real parse — so a YAML recipe supplies the base config while any flag still explicitly
    # passed on the command line overrides it (argparse always prefers an explicit flag over
    # set_defaults).
    config_path = parser.parse_known_args()[0].config
    if config_path:
        apply_yaml_defaults(parser, config_path)
    args = parser.parse_args()
    from nano_swe.utils.config import hierarchize
    ...
    if args.algo.advantage.estimator == "on_policy_distill":
        ...
        if not args.train.agent_path:
            ...
            from nano_swe.agents import distill_agent
