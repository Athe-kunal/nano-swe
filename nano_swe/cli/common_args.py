...
def apply_yaml_defaults(parser: argparse.ArgumentParser, config_path: str) -> None:
    """Loads a nested YAML recipe and applies it as this parser's defaults.

    Nested keys flatten to the same dotted dest names argparse already uses (e.g.
    ``actor: {model_name_or_path: ...}`` -> ``actor.model_name_or_path``), so a flag also passed
    explicitly on the command line still overrides the YAML value.
    """
    ...
    parser.set_defaults(**flatten(config))
