...
from nano_swe.trainer.placement import get_bundle_indices, ray_noset_visible_devices
...
class RolloutRayActor:
    ...
    async def reset_prefix_cache(self):
        ...
        The defaults are not enough here. We pause with mode="keep", so requests that
        straddle the update keep their blocks, and a plain reset leaves exactly those
        alone — they re-enter the prefix cache when the request finishes, carrying KV
        computed under the previous policy. Ask for the running requests and any
        external KV connector too, on the vLLM versions that expose them, and say so
        when the engine still refuses (it returns False rather than raising).
        """
        import inspect

        supported = inspect.signature(self.llm.reset_prefix_cache).parameters
        kwargs = {name: True for name in ("reset_running_requests", "reset_connector") if name in supported}
        if await self.llm.reset_prefix_cache(**kwargs) is False:
            print("[refit] WARNING: vLLM refused to reset the prefix cache; rollout may reuse stale KV", flush=True)
...
def create_vllm_engines(
    ...
    for i in range(num_engines):
        ...
        actor_kwargs = {
            ...
            "worker_extension_cls": "nano_swe.trainer.vllm.vllm_worker_wrap.WorkerWrap",
